"""Real-world push-T -- the full control loop, with a PERTURBATION pause.

``push_t_realworld_run.py`` plus one thing: press **q** at any time and the loop finishes
the action it is on, then stops and waits.  Move the shape by hand, press **c**, and the
run carries on from wherever the shape now is -- see "Perturbing the shape" below.
...
Usage (inside the Docker image; from the repo root):
    ...
    python push_t_realworld_run_perturb.py --dry-run --ref plan.npy --viz

    # live: UR at --ip, RealSense tracking marker --tee-marker-id glued to the T
    python push_t_realworld_run_perturb.py --ip 192.168.10.2 --z-push 0.02 \\
        --tee-marker-id 2 --viz

Perturbing the shape
--------------------
Keys are read straight off the terminal (``docker run -ti``; stdin must be a TTY, and
without one the keys are simply off and this is the plain loop):

* **q** -- ask for a pause.  Nothing happens right away: the action in flight (retract,
  arc, re-entry, push) runs to its end, and the loop stops BEFORE the next observation,
  with the arm at rest against the shape.  A ``q`` typed while a move is running is not
  lost -- the moves are blocking, so it waits in the input buffer and is seen the moment
  the action ends.
* move the shape.  Anything goes: a few centimetres, a turn, both.
* **c** -- continue.  The loop re-reads the shape, flies the end effector out to the
  WIDEST stand-off circle around the shape's NEW pose and round it to the home point --
  ``ArmPushSession.home_move_real``, the same collision-checked fly-around the run starts
  with, on the one circle that clears the T from every angle -- and then steps as normal.
  The step re-plans the reference from the measured pose and re-seats the carrot on it,
  so no other state needs resetting: the perturbed pose is just the next observation.

Why the fly-out first, rather than letting ``step_real`` take it from there: the step's
transit ladder tries the TIGHTEST arc first and only checks its lead leg against the
polygon, which is right for a pusher that just finished a push on this very shape and
wrong for one the shape has been moved around -- the lead leg can then run through the
T's new footprint and shove it before the first replanned action is ever flown.  From the
widest circle every transit the ladder can pick is one it was designed for.
...
# ======================================================================================
# keyboard -- the perturbation pause
# ======================================================================================
PAUSE_KEY = 'q'         # finish the current action, then stop and wait
RESUME_KEY = 'c'        # re-read the shape, re-home, carry on


class KeyWatch:
    """Stdin in cbreak mode: single keys, no Enter, no echo, never blocking unless asked.

    The same trick as ``arm_calibrate.RawKeyboard``, cut down to the two keys this loop
    needs.  No thread: every arm move in this file blocks, so a key pressed mid-move
    simply waits in the terminal's input buffer and ``pressed`` sees it at the next check
    point.  Ctrl-C still raises ``KeyboardInterrupt`` -- cbreak leaves ISIG on.

    ``enabled`` is False when stdin is not a terminal (a pipe, ``docker run`` without
    ``-t``), in which case every method is a no-op and the run is the plain loop.
    """
    ...
    def __init__(self, stream=None):
        ...
        self._saved = None
        self._buf = ''          # typed but not yet consumed -- the ``c`` after a ``q``
    ...
    def _drain(self):
        """Pull every character waiting in the terminal into ``_buf``.  Never blocks."""
        while self.enabled and select.select([self.fd], [], [], 0)[0]:
    ...
    def _take(self, key):
        """Consume the buffer up to and including the first ``key``; True if it was there.

        What comes AFTER the key is kept: ``q`` then ``c`` typed while an action is still
        running arrive in one read, and the ``c`` is the resume that ``wait_for`` is about
        to look for.  What comes before it (or all of it, when the key is absent) is
        discarded -- nothing else typed means anything while the loop is running.
        """
        self._drain()
        i = self._buf.find(key)
        if i < 0:
            self._buf = ''
            return False
    ...
    def pressed(self, key):
        """True if ``key`` was typed since the last check.  Never blocks."""
        return self._take(key)

    def wait_for(self, key):
        """Block until ``key`` is typed.  Anything else is ignored."""
        if not self.enabled:
            return
        while not self._take(key):
...
class RunResult:
    ...
    perturbations: int = 0          # how many q / move / c pauses the run went through
...
def fly_home(sess: rw.ArmPushSession, arm, tee, v_transit, a_transit, verbose=True,
             why='home the end effector'):
    """Fly the end effector out to the WIDEST stand-off circle round ``tee`` and along it
    to the home point -- ``ArmPushSession.home_move_real`` from wherever the arm is now.

    Collision-free by construction: the retract leg is ``_reach_circle``'s tested ray
    (works from inside the circle -- the arm parked against the shape -- as well as from
    across the table), the ride is on the one circle that clears the T from every angle,
    and the drop is a no-op because the home point sits on that circle.  Used for the
    first move of the run and again after every perturbation, when the arm is standing
    where the shape USED to be and the step loop's tight-first ladder is the wrong tool.
    """
    plan = sess.home_move_real(tee, arm.tcp_xy_cm(), robot=sess.robot, accel=a_transit)
    home = np.asarray(plan.waypoints_real[-1], dtype=float)
    if verbose:
        print(f'[run] T=({tee[0]:.1f},{tee[1]:.1f},{math.degrees(tee[2]):+.0f}deg)  '
              f'{why} -> ({home[0]:.1f},{home[1]:.1f}) cm'
              f'  ARC r={plan.transit_radius / rw.sim_units_per_cm():.1f}cm widest')
    if plan.robot_move is None:                       # no RobotFrame: nothing to fly
...
def run_pipeline(sess: rw.ArmPushSession, arm, tracker, *, settle_s=0.6,
                 ...
                 keys: "KeyWatch | None" = None):
    """Park at the corner, home, then step until close enough / blocked / out of budget.

    ``keys`` is the perturbation pause (see the module docstring).  ``PAUSE_KEY`` is
    checked once per iteration, BEFORE the observation -- i.e. after the previous action
    has run to its end -- so a press anywhere in an action lands between that action and
    the next.  On ``RESUME_KEY`` the shape is re-read, the arm is re-homed round its new
    pose with ``fly_home``, and the iteration goes on as if nothing had happened: the
    step re-plans from the fresh reading.  A pause does not spend an action of the
    budget.  ``None`` (or a ``KeyWatch`` on a non-TTY stdin) disables it.
    ...
    fly_home(sess, arm, tee, v_transit, a_transit, verbose)
    if emit:
        emit(tee, arm.tcp_xy_cm(), sess)
    if keys is not None and keys.enabled and verbose:
        print(f'[run] perturbation pause armed: press "{PAUSE_KEY}" to stop after the '
              f'current action, move the shape, then "{RESUME_KEY}" to continue')

    dgoal = dang = float('nan')
    n_pause = 0
    for i in range(max_actions):
        # The perturbation pause.  Here -- after the last action ran to its end and
        # before this iteration's observation -- so the arm is at rest, the reading that
        # follows is of the shape where the hand left it, and the action that follows is
        # planned from that reading.
        if keys is not None and keys.pressed(PAUSE_KEY):
            n_pause += 1
            print(f'[run] PAUSED before action #{i:03d} (perturbation {n_pause}): move '
                  f'the shape, then press "{RESUME_KEY}" to continue')
            keys.wait_for(RESUME_KEY)
            tee = _read_tee(tracker)
            if tee is None:
                if verbose:
                    miss = getattr(tracker, 'last_miss', None) or 'no reading'
                    print(f'[run] #{i:03d}  no T after the perturbation ({miss})')
                return RunResult('NO_TEE', i, dgoal, dang, n_pause)
            # Out to the widest circle round the NEW pose before anything else is
            # planned: the arm is standing against where the shape WAS, and the step
            # loop's tight-first transit ladder assumes it is standing against where the
            # shape IS.  Any point on that circle would do; the home point is the one
            # the run itself starts from.
            fly_home(sess, arm, tee, v_transit, a_transit, verbose,
                     why='resume: re-home round the moved shape')
            if emit:
                emit(tee, arm.tcp_xy_cm(), sess)
            time.sleep(settle_s)
        ...
        if dgoal <= goal_cm and dang <= goal_deg:
            return RunResult('GOAL', i, dgoal, dang, n_pause)
        if plan.done:               # the controller is finished, the pose test is not
            return RunResult('SHORT', i, dgoal, dang, n_pause)
        if plan.robot_move is None:                        # BLOCKED (or GOAL w/o a move)
            return RunResult('BLOCKED', i, dgoal, dang, n_pause)

        arm.run(plan.robot_move)
        time.sleep(settle_s)

    return RunResult('MAX_ACTIONS', max_actions, dgoal, dang, n_pause)
...
def build_cli():
    ...
    ap.add_argument('--no-keys', action='store_true',
                    help=f'switch the perturbation pause off: normally "{PAUSE_KEY}" '
                         f'stops the loop after the current action and "{RESUME_KEY}" '
                         f're-reads the shape, re-homes the arm round its new pose and '
                         f'continues.  Off on its own whenever stdin is not a terminal')
...
def main():
    ...
    if cli.no_keys:
        keys.enabled = False
    elif not keys.enabled:
        print('[run] stdin is not a terminal -- the perturbation pause (q / c) is off')
    try:
        with keys:                     # cbreak for the run; the terminal is put back after
            res = run_pipeline(sess, arm, tracker, settle_s=cli.settle,
                               ...
                               park_corner=not cli.no_park, keys=keys)
    finally:
        arm.close()
        if hasattr(tracker, 'close'):
            tracker.close()

    print(f'\n[run] {res.status} after {res.actions} actions and {res.perturbations} '
          f'perturbation(s); final |goal| = {res.dist_cm:.1f} cm / {res.ang_deg:.1f} deg '
